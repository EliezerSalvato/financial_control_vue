import type { AuthSessionSuccessResponseApi, SessionRefreshPayload } from '@/types/auth';
...
export async function refreshSession(payload: SessionRefreshPayload) {
  const response = await apiRequest<AuthSessionSuccessResponseApi>('/api/v1/user/session/refreshes', {
    method: 'PATCH',

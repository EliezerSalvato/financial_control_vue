...
import type { CategoryGoal, CategoryGoalForm, CategoryGoalUpdateResult } from '@/types/category';
import { updateCategoryGoal } from '@/api/categories';
...
const emit = defineEmits<{
  ...
  saved: [result: CategoryGoalUpdateResult];
}>();
...
const form = reactive<CategoryGoalForm>({
  value: null,
  startsOn: null,
...
function validate(): boolean {
  ...
  if (startsOn && props.endsOn) {
    ...
    if (startsTotal != null && endsTotal != null && startsTotal > endsTotal) {
      handler.add('startsOn', t('categories.errors.startsOnAfterEndsOn'));
    }
  }
  ...
  if (form.value != null) {
    ...
    if (previousValue != null && form.value === previousValue) {
      handler.add('value', t('categories.errors.valueMustDifferFromPreviousGoal'));
    }
  }
...
async function save() {
  ...
  try {
    const result = await updateCategoryGoal(props.categoryId, {
      categoryGoal: {
        value: form.value ?? 0,
...
<template>
  <Modal :title="t('categories.changeGoal.title')" :open-modal="open" @close="close">
    <template #body>
      <form id="change-category-goal-form" @submit.prevent="save">
        <NotificationMessage v-if="errors.base.length" type="danger" @close="errors.base = []">
          <p v-for="error in errors.base" :key="error">{{ error }}</p>
        </NotificationMessage>

        <InputNumeric v-model="form.value" name="value" :label="t('categories.changeGoal.value')" required :error="errors.value[0]" />
        ...
        <div class="field">
          <Calendar
            ...
            name="startsOn"
            precision="month"
            :label="t('categories.changeGoal.startsOn')"
            required
        ...
        <div class="field">
          <div class="control">
            <CheckBox v-model="form.changeForNextMonths" name="changeForNextMonths" :label="t('categories.changeGoal.changeForNextMonths')" />
          </div>
        </div>
      </form>
    </template>
    ...
    <template #footer>
      <button type="submit" form="change-category-goal-form" class="button is-link" :class="{ 'is-loading': saving }" :disabled="saving">
        {{ t('buttons.save') }}
      </button>
